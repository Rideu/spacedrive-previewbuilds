...
{
	float4 n = sumOctave((uv.x + 0.5) * val1, (uv.y + 0.5) * val1, val1, 0.6, 3, 0.1,1.6);
	float4 n1 = sumOctave((uv.y + 5) * 0.5, (uv.x + 61) * 0.2, n.x / 1.7, 0.15, 4, 0.1,2);
	//float4 s0 = 1;
	float4 s1 = (n1 + n * 2 + 12).a;
	float4 s2 = (n1).a;
	float4 s3 = (n * 2 + 2.7).a;
	return (1 - s2.b)*float4(0,0,0,1) + s2.b*float4(0.2,0.4,0.6,1);
}
